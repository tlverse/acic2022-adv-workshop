...
combination of multiple candidates’ predictions. The `sl` object defined in the 
previous section, `sl`, is an eSL since it used NNLS as the meta-learner. 
...
automates a procedure for the final SL to be the learner that achieved the 
best CV predictive performance, i.e., lowest CV risk. If the eSL performs 
better than any other candidate, the dSL will end up selecting it. As mentioned 
in @rvp2022super, "another advantage of this approach is that multiple eSLs 
that use more flexible meta-learner methods (e.g., non-parametric machine 
learning algorithms like HAL) can be evaluated simultaneously.
